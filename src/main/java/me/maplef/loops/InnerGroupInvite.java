    ...
    public void execute(JobExecutionContext context){
        ...
        try (Statement stmt = c.createStatement();
            ...
            while(res.next()){
                ...
                if(totalTime > 600 && weeklyTime / 7 >= config.getInt("isActive-time-per-day") && !Objects.requireNonNull(bot.getGroup(innerGroup)).contains(playerQQ)){
                    ...
                    Objects.requireNonNull(Objects.requireNonNull(bot.getGroup(playerGroup)).get(playerQQ)).sendMessage("这是你的凭证：");
                    Objects.requireNonNull(Objects.requireNonNull(bot.getGroup(playerGroup)).get(playerQQ)).sendMessage(inviteCode);

                    String inviteMsg = String.format("已邀请 %s 进入内群\n", playerName);
                    inviteMsg = inviteMsg.concat("入群凭证为: ").concat(inviteCode);
